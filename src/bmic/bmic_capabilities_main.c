...
void capabilities_do_read(bmic_handle_t *handle, bmic_api_interface_t *api,
	const bmic_exchange_t *cap, const char *capname, gru_status_t *status) {
	const bmic_exchange_t *obj = api->attribute_read(handle, cap, capname, status);
...
static void capabilities_read(const void *nodedata, void *payload) {
	...
	capabilities_do_read(
		wrapper->handle, wrapper->api, wrapper->cap, info->name, wrapper->status);
}
...
int capabilities_run(options_t *options) {
	...
	if (options->program.capabilities.list) {
		...
		if (options->program.capabilities.readall) {
			...
			const bmic_exchange_t *obj = api->attribute_read(
...
int capabilities_main(int argc, char **argv) {
	...
	if (options.program.capabilities.list == false &&
		options.program.capabilities.read == NULL &&
