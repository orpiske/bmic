...
static void show_help(char **argv) {
	...
	gru_cli_option_help("name [name]", "n [name]", "name of the queue to manage");
	gru_cli_option_help("attribute [attribute]", "a [attribute]", "queue attribute to read");
...
static void print_topic(const void *nodedata, void *payload) {
	const char *name = (const char *) nodedata;

	printf("Topic name: %s\n", name);
}

static void print_topic_stat(const char *name, bmic_topic_stat_t stat) {
	printf("Size: %" PRId64 "\n", stat.queue_size);
	printf("Consumers: %" PRId64 "\n", stat.consumer_count);
	printf("Enqueue Count: %" PRId64 "\n", stat.msg_enq_count);
	printf("Dequeue Count: %" PRId64 "\n", stat.msg_deq_count);
	printf("Delta: %" PRId64 "\n", stat.msg_enq_count - stat.msg_deq_count);
}

int topic_run(options_t *options) {
	...
	switch (options->program.queue.operation) {
		...
		case OP_LIST: {
			const bmic_list_t *list = api->topic_list(ctxt.handle, cap, &status);

			if (list) {
				gru_list_for_each(list->items, print_topic, NULL);
