...
static void bmic_activemq_mi_translate_arg_value(const void *nodedata, void *payload) {
...
static void bmic_activemq_mi_translate_sigs(const void *nodedata, void *payload) {
    const bmic_object_t *node = (const bmic_object_t *) nodedata;
    ...
    if (args && args->type == LIST) {
        bmic_object_for_each_child(args, bmic_activemq_mi_translate_arg_value, 
                                   sig);
    }
...
{
    if (obj->type == LIST) {
        bmic_object_for_each_child(obj, bmic_activemq_mi_translate_sigs, info);
    }
    else {
        bmic_activemq_mi_translate_sigs(obj, info);
    }
}